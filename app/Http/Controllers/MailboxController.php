...
use App\Models\Models\MailboxFolder;
use App\Models\Models\Mailbox;
use App\Models\Models\MailboxAttachment;
use App\Models\Models\MailboxFlags;
use App\Models\Models\MailboxReceiver;
use App\Models\Models\MailboxTmpReceiver;
use App\Models\Models\MailboxUserFolder;
use App\Models\User;
    ...
    private function getData($keyword, $perPage, $foldername)
    {
        ...
        if($foldername == "Inbox") {
            $query = Mailbox::join('mailbox_receivers', 'mailbox_receivers.mailbox_id', '=', 'mailboxes.id')
                    ->join('mailbox_user_folders', 'mailbox_user_folders.user_id', '=', 'mailbox_receivers.receiver_id')
                    ->join('mailbox_flags', 'mailbox_flags.user_id', '=', 'mailbox_user_folders.user_id')
                    ->where('mailbox_receivers.receiver_id', Auth::user()->id)
                    ->where('mailbox_user_folders.folder_id', $folder->id)
                    ->where('sender_id', '!=', Auth::user()->id)
//                    ->where('parent_id', 0)
                    ->whereRaw('mailboxes.id=mailbox_receivers.mailbox_id')
                    ->whereRaw('mailboxes.id=mailbox_flags.mailbox_id')
                    ->whereRaw('mailboxes.id=mailbox_user_folders.mailbox_id')
                    ->select(["*", "mailboxes.id as id", "mailbox_flags.id as mailbox_flag_id", "mailbox_user_folders.id as mailbox_folder_id"]);
        } else if ($foldername == "Sent" || $foldername == "Drafts") {
            $query = Mailbox::join('mailbox_user_folders', 'mailbox_user_folders.mailbox_id', '=', 'mailboxes.id')
                ->join('mailbox_flags', 'mailbox_flags.user_id', '=', 'mailbox_user_folders.user_id')
                ->where('mailbox_user_folders.folder_id', $folder->id)
                ->where('mailbox_user_folders.user_id', Auth::user()->id)
//                ->where('parent_id', 0)
                ->whereRaw('mailboxes.id=mailbox_flags.mailbox_id')
                ->whereRaw('mailboxes.id=mailbox_user_folders.mailbox_id')
                ->select(["*", "mailboxes.id as id", "mailbox_flags.id as mailbox_flag_id", "mailbox_user_folders.id as mailbox_folder_id"]);
            ...
            $query = Mailbox::join('mailbox_user_folders', 'mailbox_user_folders.mailbox_id', '=', 'mailboxes.id')
                ->join('mailbox_flags', 'mailbox_flags.user_id', '=', 'mailbox_user_folders.user_id')
                ->leftJoin('mailbox_receivers', 'mailbox_receivers.mailbox_id', '=', 'mailboxes.id')
                ->where(function ($query) {
                    $query->where('mailbox_user_folders.user_id', Auth::user()->id)
...
//                ->where('parent_id', 0)
                ->whereRaw('mailboxes.id=mailbox_flags.mailbox_id')
                ->whereRaw('mailboxes.id=mailbox_user_folders.mailbox_id')
                ->whereRaw('mailbox_user_folders.user_id!=mailbox_receivers.receiver_id')
                ->select(["*", "mailboxes.id as id", "mailbox_flags.id as mailbox_flag_id", "mailbox_user_folders.id as mailbox_folder_id"]);
        }
        if (!empty($keyword)) {
            $query->where('subject', 'like', "%$keyword%");
        }
        $query->orderBy('mailboxes.id', 'DESC');
